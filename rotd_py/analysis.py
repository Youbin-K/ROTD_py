    ...
    for i, line in enumerate(lines):
        if line.startswith('Microcanonical'):
            for j in range(0, e_size):
                e_flux[j] += float(lines[i+1+j].split()[1 + pes_index]) #1+ (pes_index = 0) 이니까 결국 1이 되서 2번째 column 읽어서 e_flux에 넣음
    return e_flux
...
def get_ej_flux(file_name, e_size, j_size, pes_index):
    # read through the file and return the total thermal flux
    # this function does not separate the contribution from each facet
    ej_flux = np.zeros(e_size, j_size) #2D array
    lines = open(file_name).readlines()
    for k, line in enumerate(lines):
        if line.startswith('E-J resolved'):
            for i in range(0, e_size):
                for j in range(0, j_size): # data가 1줄이어서 2D array 채우려고 for 문 2개 i,j
                    ej_flux[i][j] += float(lines[k+i*j_size+j].split()[pes_index]) # 결국 이게 나오는건 .dat 파일에서 microcanonical 밑의 
...
def integrate_micro(e_flux, energy_grid, temperature_grid, dof_num):
    ...
    energy_grid = energy_grid * rotd_math.Kelv # in units of Kelv. rotd_math.kelv = kelv/Hartree
    temperature_grid = temperature_grid * rotd_math.Kelv # in units of Kelv.
    temper_fac = np.power(temperature_grid, dof_num/2)
    if dof_num % 2:
        temper_fac *= np.sqrt(temperature_grid) # 무조건 루트가 나오게
