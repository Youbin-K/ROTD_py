...
class OneDimensional(Correction):
    ...
    def generate(self, parameters:dict):
        ...
        if parameters["r_sample"] == None or\
        not isinstance(parameters["r_sample"], list):
            raise AttributeError("1D correction needs a 'r_sample' entry (list).")
        else:
            self.r_sample = parameters["r_sample"]
        if parameters["e_sample"] == None or\
        not isinstance(parameters["e_sample"], list):
            raise AttributeError("1D correction needs a 'e_sample' entry (list).")
        else:
            self.e_sample = parameters["e_sample"]
        if parameters["r_trust"] == None or\
        not isinstance(parameters["r_trust"], list):
            raise AttributeError("1D correction needs a 'r_trust' entry (list).")
        else:
            self.r_trust = parameters["r_trust"]
        if parameters["e_trust"] == None or\
        not isinstance(parameters["e_trust"], list):
            raise AttributeError("1D correction needs a 'e_trust' entry (list).")
        else:
        ...
        spln_sample = make_interp_spline(self.r_sample,
                                         np.asarray(self.e_sample)*rotd_math.Kcal*rotd_math.Hartree)
        spln_trust = make_interp_spline(self.r_trust,
                                        np.asarray(self.e_trust)*rotd_math.Kcal*rotd_math.Hartree)
