...
with connect('../rotd.db', timeout=60) as cursor:
    ...
    row = cursor.execute(sql_cmd, ({surf_id}, {face_id}, {samp_id})).fetchall()[-1]

# Transform db data into Flux-type data.
int_flux_tag, srl_flux, surf_id, face_id, samp_len, samp_id, status = row
...
with connect('../rotd.db', timeout=60) as cursor:
    cursor.execute('UPDATE fluxes SET flux=:srl_flux, status=:status '
                   'WHERE surf_id = :surf_id AND face_id = :face_id AND samp_id = :samp_id',
                   {{'srl_flux': srl_flux, 'status': 'COMPLETED', 'surf_id': surf_id, 'face_id': face_id, 'samp_id': samp_id}})
"""  # TODO Uncomment
