...
class MultiSample(Sample):
    ...
    def generate_configuration(self):
        ...
        # print (repr(second_carbon_position_after_labframe),'],') # Turn on only for gas
        # print (repr(top_middle_platinum_position_after_labframe),'],') # Turn on only for surface

        # For Surface Coordinate after conversion
        # 변환 한뒤의 좌표
        ...
        if there_is_slab:
            for index, frag in enumerate(self.fragments):
                if frag.molecule_type == MolType.MONOATOMIC:
                ...
                elif frag.molecule_type == MolType.NONLINEAR: # 이거 molframe 으로 바꾸기!!!!! -> gas 에서는 어차피 시작부터 몰프레임이라 기존처럼 줘도 상관없지만
                                                              # Surface의 경우 absolute coordinate 주어지기에 바꿔줘야함
                    ...
                    mf_com = frag.lf2mf(com_vec)
                    if index == 0:
                        rc_vec = np.append(rc_vec, np.cross(molframe_pivot, mf_com)) # np.cross = multiplication of vectors
                        #print ("rc_vec index ==0", rc_vec)
                    else: # index == 1
                        rc_vec = np.append(rc_vec, np.cross(mf_com, molframe_pivot))
                    # print ('slab inertia', frag.get_inertia_moments_for_surface())
                    # continue
        else:
            for index, frag in enumerate(self.fragments):
                if frag.molecule_type == MolType.MONOATOMIC:
